...
import { getAlarmmsgs, exportList } from '@/api/smartPass/alarm/vehicle'
...
export default function E(props) {
  ...
  const toDetail = result => {
    history.push(`/smartPass/alarm/vehicleAlarm/detail/${result.id}`)
  }
  ...
  return (
    <div>
      <ComponentTitle title='车辆告警' />
      <TablePage
        searchConfig={searchConfig}
        columns={columns}
        getList={getAlarmmsgs}
        propsCriteria={propsCriteria}
        exportFile={{ exportMethod: exportList, fileName: '车辆告警记录.xlsx' }}
      >
        <ButtonGroup>
          <div>
