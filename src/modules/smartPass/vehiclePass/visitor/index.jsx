...
import { getVisitors, exportList } from '@/api/smartPass/vehiclePass/visitor'
...
function Element(props) {
  ...
  const toAllRecords = result => {
    history.push(`/smartPass/vehiclePass/visitorVehicle/detail/${result.refId}`)
  }
  ...
  const columns = [
    { title: '车牌号', dataIndex: 'visitorCarNo' },
  ...
  return (
    <div>
      <ComponentTitle title='访客车辆' />
      <TablePage
        searchConfig={searchConfig}
        columns={columns}
        getList={getVisitors}
        propsCriteria={propsCriteria}
        exportFile={{ exportMethod: exportList, fileName: '访客车辆记录.xlsx' }}
      >
        <ButtonGroup>
          <div>
            <span style={{ fontSize: 16, paddingLeft: 16, fontWeight: 600, color: '#14579d' }}>访客车辆记录</span>
          </div>
        </ButtonGroup>
      </TablePage>
